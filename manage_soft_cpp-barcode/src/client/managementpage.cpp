...
void ManagementPage::buildUi()
{
    ...
    if (isInventoryPage()) {
        ...
        auto *scanStockOutButton = new QPushButton(QStringLiteral("扫码出库"), this);
        ...
        setButtonVariant(scanStockOutButton, QStringLiteral("primary"));
        ...
        connect(scanStockOutButton, &QPushButton::clicked, this, [this]() {
            scanInventoryRecord(InventoryOperationType::StockOut);
        });
        connect(scanStockInButton, &QPushButton::clicked, this, [this]() {
            scanInventoryRecord(InventoryOperationType::StockIn);
        });
        ...
        manualLayout->addWidget(scanStockOutButton);
...
void ManagementPage::scanInventoryRecord(InventoryOperationType operationType)
{
    const bool stockIn = operationType == InventoryOperationType::StockIn;
    bool accepted = false;
    const QString barcode = QInputDialog::getText(
                                this,
                                stockIn ? QStringLiteral("扫码入库") : QStringLiteral("扫码出库"),
                                QStringLiteral("请扫描物料条码（扫码枪回车后会自动继续）："),
                                QLineEdit::Normal,
                                QString(),
                                &accepted)
                                .trimmed();
    ...
    if (barcode.isEmpty()) {
        QMessageBox::warning(this, QStringLiteral("条码无效"), QStringLiteral("没有读取到条码，请重新扫描。"));
        return;
    }
    ...
    for (const QVariantMap &record : inventoryRecords) {
        if (record.value(QStringLiteral("barcode")).toString().trimmed() == barcode) {
            matches.append(record);
        }
    }
    ...
    if (matches.isEmpty()) {
        QMessageBox::information(
            this,
            QStringLiteral("未找到物料"),
            QStringLiteral("条码 %1 尚未绑定库存物料。请先新增或编辑物料，并填写该条码。").arg(barcode));
        return;
    }
    if (matches.size() > 1) {
        QMessageBox::critical(
            this,
            QStringLiteral("条码重复"),
            QStringLiteral("条码 %1 匹配到多条库存记录，请先修正重复条码。").arg(barcode));
        return;
    }

    const QVariantMap record = matches.first();
    InventoryTransactionDialog dialog(stockIn ? InventoryTransactionDialog::Mode::StockIn
                                              : InventoryTransactionDialog::Mode::StockOut,
    ...
    if (!m_storageService->applyInventoryChange(changeRecord,
                                                ...
                                                combineInventoryNotes(QStringLiteral("扫码枪"), dialog.note()),
                                                QString(),
                                                0,
                                                &errorMessage)) {
        QMessageBox::critical(this,
                              stockIn ? QStringLiteral("扫码入库失败") : QStringLiteral("扫码出库失败"),
                              errorMessage);
        return;
    }
    ...
    QMessageBox::information(this,
                             QStringLiteral("操作成功"),
                             stockIn ? QStringLiteral("扫码入库已完成。") : QStringLiteral("扫码出库已完成。"));
}
